    ...
    <section style={{ maxWidth: 'var(--container-max)', margin: '0 auto', padding: '36px var(--gutter) 0' }}>
      <div data-reveal={reveal ? '' : undefined} style={{
        ...
        <div className="ember-zoom" style={{ position: 'absolute', inset: 0, overflow: 'hidden' }}>
          <image-slot id="ember-hero" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%' }} shape="rect" fit="cover" placeholder="{{HERO_IMAGE}}"></image-slot>
        </div>
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 560, minHeight: isMobile ? 420 : 480, padding: isMobile ? '32px 26px' : '56px 52px', display: 'flex', flexDirection: 'column', justifyContent: isMobile ? 'flex-end' : 'center' }}>
          <span className="ember-rise" style={{ display: 'inline-block', animationDelay: '0.15s' }}><Badge tone="gold">{'{{HERO_BADGE}}'}</Badge></span>
          ...
          <div className="ember-rise" style={{ animationDelay: '0.54s', display: 'flex', gap: 12, flexWrap: 'wrap' }}>
            <Button variant="primary" size={isMobile ? 'md' : 'lg'} onClick={onShop}>{'{{CTA_PRIMARY}}'}</Button>
            <Button variant="outline" size={isMobile ? 'md' : 'lg'} onClick={onShop}>{'{{CTA_SECONDARY}}'}</Button>
          </div>
        </div>
      </div>
    </section>
...
function CategoryShowcase({ onPick }) {
  const cats = [
    { id: 'cat-tile-1', name: '{{CATEGORY_TILE_1_NAME}}', note: '{{CATEGORY_1}}', pick: 'cat-1', img: '' },
    { id: 'cat-tile-2', name: '{{CATEGORY_TILE_2_NAME}}', note: '{{CATEGORY_1}}', pick: 'cat-1', img: '' },
    { id: 'cat-tile-3', name: '{{CATEGORY_TILE_3_NAME}}', note: '{{CATEGORY_2}}', pick: 'cat-2', img: '' },
    { id: 'cat-tile-4', name: '{{CATEGORY_TILE_4_NAME}}', note: '{{CATEGORY_2}}', pick: 'cat-2', img: '' },
  ];
  return (
    <section style={{ maxWidth: 'var(--container-max)', margin: '0 auto', padding: '64px var(--gutter) 0' }}>
      <SectionHead eyebrow="{{SHOWCASE_EYEBROW}}" title="{{SHOWCASE_TITLE}}" />
...
function PromoBanners({ onPick }) {
  ...
  const isMobile = window.useIsMobile();
  const banners = [
    { id: 'promo-1', eyebrow: '{{PROMO_1_EYEBROW}}', title: '{{PROMO_1_TITLE}}', cta: '{{PROMO_1_CTA}}', cat: 'cat-2' },
    { id: 'promo-2', eyebrow: '{{PROMO_2_EYEBROW}}', title: '{{PROMO_2_TITLE}}', cta: '{{PROMO_2_CTA}}', cat: 'cat-1' },
  ];
  return (
    <section style={{ maxWidth: 'var(--container-max)', margin: '0 auto', padding: '64px var(--gutter) 0' }}>
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(2, 1fr)', gap: 18 }}>
        {banners.map(b => (
          <div key={b.id} style={{ position: 'relative', borderRadius: 'var(--radius-lg)', overflow: 'hidden', border: '1px solid var(--line)', minHeight: 240 }}>
            <image-slot id={b.id} style={{ width: '100%', height: '100%', minHeight: 240 }} shape="rect" fit="cover" placeholder="{{BANNER_IMAGE}}"></image-slot>
            <div style={{
              position: 'absolute', inset: 0, pointerEvents: 'none',
